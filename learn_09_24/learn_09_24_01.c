...
//#define CONFIG_FILE "config.txt"  // 配置文件名称
...
//
//// 函数声明
//void loadConfig(Config* config);                // 加载配置文件
//void saveConfig(Config* config);                // 保存配置文件
//int loginToCampusNetwork(Config* config);       // 登录校园网的函数
//
//int main() {
...
//        if (loginToCampusNetwork(&config)) {   // 调用登录函数
//            if (config.showSuccessMessage) {   // 如果设置了显示成功信息
//                printf("登录成功！\n");         // 输出成功提示
//            }
//            return 0;                          // 成功后退出程序
//        }
...
//    }
//
//    printf("尝试登录失败 %d 次。\n", MAX_ATTEMPTS); // 超过最大尝试次数的提示
//    char choice;                               // 用户选择变量
//    printf("是否需要更改登录信息？(y/n): ");    // 提示用户是否需要更改登录信息
//    scanf(" %c", &choice);                     // 读取用户输入
//    if (choice == 'y' || choice == 'Y') {      // 如果用户选择是
//        printf("请输入新的用户名: ");           // 提示输入新用户名
//        scanf("%s", config.username);           // 读取新用户名
//        printf("请输入新的密码: ");             // 提示输入新密码
//        scanf("%s", config.password);           // 读取新密码
//        saveConfig(&config);                    // 保存新配置
//    }
//
//    return 0;                                   // 程序结束
//}
...
//    if (file) {                                 // 如果文件存在
//        fscanf(file, "%s %s %s %d %d", config->username,
//            config->password, config->line,
//            &config->showSuccessMessage, &config->autoLogin); // 读取配置
//        fclose(file);                           // 关闭文件
//    }
//    else {
//        // 如果配置文件不存在，设置默认值
//        strcpy(config->username, "2021300027"); // 默认用户名
//        strcpy(config->password, "APTX-4869");  // 默认密码
//        strcpy(config->line, "unicom");         // 默认登录线路
//        config->showSuccessMessage = 1;         // 默认显示成功提示
//        config->autoLogin = 1;                  // 默认允许自动登录
//    }
//}
//
//// 保存配置文件
...
//    fprintf(file, "%s\n%s\n%s\n%d\n%d\n", config->username,
//        config->password, config->line,
//        config->showSuccessMessage, config->autoLogin); // 保存配置
//    fclose(file);                               // 关闭文件
//}
//
//// 登录函数
...
//    if (curl) {
//        curl_easy_setopt(curl, CURLOPT_URL, url);              // 设置请求 URL
//
//        // 直接输出响应到标准输出，而不是存储
...
//    printf("尝试登录失败 %d 次。\n", MAX_ATTEMPTS); // 超过最大尝试次数的提示
//    char choice;                               // 用户选择变量
//    printf("是否需要修改账户信息？(y/n): ");    // 提示用户是否需要更改登录信息
//    scanf(" %c", &choice);                     // 读取用户输入
//    if (choice == 'y' || choice == 'Y') {      // 如果用户选择是
//        handleUserInputConfig(&config);         // 处理用户输入配置
//        attempts = 0;                           // 重置尝试计数
//        while (attempts < MAX_ATTEMPTS) {      // 再次尝试登录
//            if (loginToCampusNetwork(&config)) {
//                printf("登录成功！\n");         // 输出成功提示
//                return 0;                       // 成功后退出程序
//            }
//            else {
